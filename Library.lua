...
local ModeSelectOuter = Library:Create('Frame', {
            ...
            Size = UDim2.new(0, 90, 0, 45 + 15 + (Info.Removable and 15 or 0) + 2);
            Visible = false;
            ZIndex = 14;
            Parent = ScreenGui;
        });
...
local ContainerAccentBar = Library:Create('Frame', {
        ...
        Library:Create('UICorner', {
            ...
            Parent = ContainerAccentBar;
        });

        Library:AddToRegistry(ContainerAccentBar, {
...
ContainerLabel.Text = string.format('[%s] %s (%s)', KeyPicker.Value, Info.Text, KeyPicker.Mode);
            ...
            TweenService:Create(ContainerRow, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                BorderColor3 = TargetBorder;
            }):Play();

            TweenService:Create(ContainerAccentBar, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
Library:OnHighlight(ToggleRegion, ToggleStroke,
 ...
 local ToggleContextOuter = Library:Create('Frame', {
        ...
        local ToggleContextInner = Library:Create('Frame', {
            BackgroundColor3 = Library.BackgroundColor;
            BorderColor3 = Library.OutlineColor;
            BorderMode = Enum.BorderMode.Inset;
            Size = UDim2.new(1, 0, 1, 0);
            ZIndex = 51;
            Parent = ToggleContextOuter;
        });

        Library:AddToRegistry(ToggleContextInner, {
            BackgroundColor3 = 'BackgroundColor';
            BorderColor3 = 'OutlineColor';
        });
...
if Toggle.KeybindPicker then
        ...
        Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and ToggleContextOuter.Visible then
                if not Library:IsMouseOverFrame(ToggleContextOuter) then
                    ToggleContextOuter.Visible = false;
                end;
            end;
        end));
...
Slider:Display();
        ...
        do
            ...
            local RandOuter = Library:Create('Frame', {
                ...
                BorderSizePixel = 0;
                Position = UDim2.fromOffset(0, 0);
                Size = UDim2.fromOffset(220, 82);
                Visible = false;
                ZIndex = 15;
                Parent = ScreenGui;
            })
            Library:Create('UICorner', { CornerRadius = UDim.new(0, 8); Parent = RandOuter })
            ...
            Library:Create('UICorner', { CornerRadius = UDim.new(0, 7); Parent = RandInner })
            ...
            local ToggleShadeFrame = Library:Create('Frame', {
                BackgroundColor3 = Color3.new(0, 0, 0);
                BorderSizePixel = 0;
                Size = UDim2.new(1, 0, 1, 0);
                ZIndex = 19;
            ...
            -- One bar. Left handle = min. Right handle = max.
            -- The filled region between them is accent-colored.
            -- Min/max values shown as floating labels above each handle.
            ...
            Library:AddToRegistry(TrackOuter, { BorderColor3 = 'Black' })
            ...
            -- Shade gradient over the whole track (same as normal sliders)
            local TrackShade = Library:Create('Frame', {
                BackgroundColor3 = Color3.new(0, 0, 0);
                BorderSizePixel = 0;
                Size = UDim2.new(1, 0, 1, 0);
                ZIndex = 18;
            ...
            -- Accent fill between the two handles
            ...
            Library:AddToRegistry(RangeFill, { BackgroundColor3 = 'AccentColor' })

            -- Shade on the fill too
            local FillShade = Library:Create('Frame', {
                BackgroundColor3 = Color3.new(0, 0, 0);
                BorderSizePixel = 0;
                Size = UDim2.new(1, 0, 1, 0);
            ...
            -- Floating value labels that sit above each handle
            -- They live in RandInner so they aren't clipped by the track
            local MinValLabel = Library:CreateLabel({
                AnchorPoint = Vector2.new(0.5, 1);
                Position = UDim2.fromOffset(8, 44);
                Size = UDim2.new(0, 40, 0, 12);
                TextSize = 11;
                Text = tostring(Slider.RandomMin);
                TextXAlignment = Enum.TextXAlignment.Center;
                ZIndex = 23;
                Parent = RandInner;
            })

            local MaxValLabel = Library:CreateLabel({
                AnchorPoint = Vector2.new(0.5, 1);
                Position = UDim2.fromOffset(8, 44);
                Size = UDim2.new(0, 40, 0, 12);
                TextSize = 11;
                Text = tostring(Slider.RandomMax);
                TextXAlignment = Enum.TextXAlignment.Center;
                ZIndex = 23;
                Parent = RandInner;
            })

            -- ── Update visuals from current RandomMin / RandomMax ────
            local function UpdateRange()
                local trackW = TrackInner.AbsoluteSize.X
                if trackW == 0 then return end

                local span = Info.Max - Info.Min
                local minRatio = (Slider.RandomMin - Info.Min) / span
                local maxRatio = (Slider.RandomMax - Info.Min) / span

                local minPx = minRatio * trackW
                local maxPx = maxRatio * trackW

                -- Accent fill between handles
                RangeFill.Position = UDim2.new(0, minPx, 0, 0)
                RangeFill.Size     = UDim2.new(0, math.max(maxPx - minPx, 0), 1, 0)

                -- Handle positions
                MinHandle.Position = UDim2.new(0, minPx, 0.5, 0)
                MaxHandle.Position = UDim2.new(0, maxPx, 0.5, 0)

                -- Floating labels (offset from track top in RandInner space)
                -- TrackOuter sits at Y=46 inside RandInner
                local trackTopInPanel = 46
                MinValLabel.Position = UDim2.fromOffset(
                    8 + minPx,       -- 8 = TrackOuter left padding
                    trackTopInPanel - 2
                )
                MaxValLabel.Position = UDim2.fromOffset(
                    8 + maxPx,
                    trackTopInPanel - 2
                )

                MinValLabel.Text = tostring(Slider.RandomMin)
                MaxValLabel.Text = tostring(Slider.RandomMax)
            end

            -- ── Drag helpers ─────────────────────────────────────────
            local function PxToVal(rawX)
                local minX = TrackInner.AbsolutePosition.X
                local w    = TrackInner.AbsoluteSize.X
                local t    = math.clamp(rawX - minX, 0, w) / w
                return RoundVal(Info.Min + t * (Info.Max - Info.Min))
            end

            local DraggingMin = false
            local DraggingMax = false

            -- Decide which handle to grab based on click proximity
            TrackInner.InputBegan:Connect(function(Input)
                if Input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end

                local trackW = TrackInner.AbsoluteSize.X
                local span   = Info.Max - Info.Min
                local minPx  = ((Slider.RandomMin - Info.Min) / span) * trackW
                local maxPx  = ((Slider.RandomMax - Info.Min) / span) * trackW
                local clickX = Mouse.X - TrackInner.AbsolutePosition.X

                local distMin = math.abs(clickX - minPx)
                local distMax = math.abs(clickX - maxPx)

                if distMin <= distMax then
                    DraggingMin = true
                    while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                        local v = PxToVal(Mouse.X)
            ...
            Library:OnHighlight(TrackOuter, TrackOuter,
                { BorderColor3 = 'AccentColor' },
                { BorderColor3 = 'Black' }
            )
            ...
            local function OpenRandomizer()
                ...
                -- Defer so AbsoluteSize is valid before we place handles
                task.defer(UpdateRange)
            end
...
local ListInner = Library:Create('Frame', {
        ...
        local ListHighlight = Library:Create('Frame', {
            BackgroundColor3 = Library.AccentColor;
            BorderSizePixel = 0;
        ...
        Library:Create('UICorner', {
            CornerRadius = UDim.new(1, 0);
            Parent = ListHighlight;
        });

        Library:AddToRegistry(ListHighlight, {
            BackgroundColor3 = 'AccentColor';
        });
...
local KeybindLabelWidth = select(1, Library:GetTextBounds('Keybinds', Library.Font, 14)) + 10;
    ...
    local KeybindHighlightLeft = Library:Create('Frame', {
        ...
        Size = UDim2.new(0.5, -(KeybindLabelWidth / 2), 0, 2);
        ZIndex = 102;
        Parent = KeybindInner;
    });

    local KeybindHighlightRight = Library:Create('Frame', {
        BackgroundColor3 = Library.AccentColor;
        BorderSizePixel = 0;
        Position = UDim2.new(0.5, (KeybindLabelWidth / 2), 0, 8);
        Size = UDim2.new(0.5, -(KeybindLabelWidth / 2), 0, 2);
        ZIndex = 102;
        Parent = KeybindInner;
    });

    Library:AddToRegistry(KeybindHighlightLeft, {
        BackgroundColor3 = 'AccentColor';
    }, true);

    Library:AddToRegistry(KeybindHighlightRight, {
        BackgroundColor3 = 'AccentColor';
    }, true);
...
if typeof(Config.Position) ~= 'UDim2' then Config.Position = UDim2.fromOffset(175, 60) end
    if typeof(Config.Size) ~= 'UDim2' then Config.Size = UDim2.fromOffset(520, 620) end
...
local TabRowY = HasSubtitle and 6 or 7;

    local TabArea = Library:Create('Frame', {
        BackgroundTransparency = 1;
        AnchorPoint = Vector2.new(1, 0);
        Position = UDim2.new(1, -12, 0, TabRowY);
        Size = UDim2.new(0, 0, 0, 16);
        AutomaticSize = Enum.AutomaticSize.X;
        ZIndex = 1;
        Parent = Inner;
...
local TabListLayout = Library:Create('UIListLayout', {
    ...
    local TabUnderline = Library:Create('Frame', {
        BackgroundColor3 = Library.AccentColor;
        BorderSizePixel = 0;
        Position = UDim2.new(0, 0, 0, TabRowY + 19);
...
TabButtonLabel.TextColor3 = Library.AccentColor;
            ...
            local TargetX = TabButton.AbsolutePosition.X - Inner.AbsolutePosition.X;
            local TargetW = TabButton.AbsoluteSize.X;

            if Window.UnderlineInitialized then
                TweenService:Create(TabUnderline, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                    Position = UDim2.new(0, TargetX, 0, TabUnderline.Position.Y.Offset);
                    Size = UDim2.new(0, TargetW, 0, 2);
                }):Play();
            else
                TabUnderline.Position = UDim2.new(0, TargetX, 0, TabUnderline.Position.Y.Offset);
                TabUnderline.Size = UDim2.new(0, TargetW, 0, 2);
                Window.UnderlineInitialized = true;
            end;
